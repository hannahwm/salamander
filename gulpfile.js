...
gulp.task('sass', function() {
  return gulp.src('app/scss/main.scss') // Gets all files ending with .scss in app/scss
    ...
    .pipe(gulp.dest('dist/interactive/2019/08/salamander/css'))
...
gulp.task('scripts', function() {
    return gulp.src('app/js/plugins/**/*.js')
        ...
        // .pipe(uglify())
        .pipe(gulp.dest('dist/interactive/2019/08/salamander/js'))
        .pipe(browserSync.reload({
          stream: true
        }))
...
gulp.task('copyJSLibraries', function() {
  return gulp.src('app/js/library/**/*.js')
    .pipe(gulp.dest('dist/interactive/2019/08/salamander/js/library'))
    .pipe(browserSync.reload({
      stream: true
    }))
...
gulp.task('copyFonts', function() {
  return gulp.src('app/fonts/**/*')
    .pipe(gulp.dest('dist/interactive/2019/08/salamander/fonts'))
    .pipe(browserSync.reload({
      stream: true
    }))
...
gulp.task('video', function() {
  return gulp.src('app/video/**/*.mp4')
    .pipe(gulp.dest('dist/interactive/2019/08/salamander/video'))
    .pipe(browserSync.reload({
      stream: true
    }))
...
gulp.task('images', function(){
  ...
  .pipe(cache(imagemin()))
  .pipe(gulp.dest('dist/interactive/2019/08/salamander/images'))
  .pipe(browserSync.reload({
    stream: true
  }))
